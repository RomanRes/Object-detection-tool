...
def darknet_residual_block(x, filters, idx):
    shortcut = x
    x = darknet_conv(
        x, filters=filters, kernel_size=(1, 1), strides=(1, 1), padding="same", idx=idx
    )
    x = darknet_conv(
        x,
        filters=filters * 2,
...
def yolo_end(x, filters, idx):
    x = darknet_conv(
        x, filters=filters, kernel_size=(3, 3), strides=(1, 1), padding="same", idx=idx
    )
    x = darknet_conv(
        x,
        filters=255,
...
def yoloV3(name=None):
    ...
    # 0 --> 4
    x = darknet_conv(
        inputs, filters=32, kernel_size=(3, 3), strides=(1, 1), padding="same", idx=0
    )
    x = darknet_conv(
        x, filters=64, kernel_size=(3, 3), strides=(2, 2), padding="valid", idx=1
    )
    x = darknet_residual_block(x, filters=32, idx=2)
    x = darknet_conv(
        x, filters=128, kernel_size=(3, 3), strides=(2, 2), padding="valid", idx=5
    )
    ...
    # 12
    x = darknet_conv(
        x, filters=256, kernel_size=(3, 3), strides=(2, 2), padding="valid", idx=12
    )
    # 13 --> 36
    for i in range(8):
        idx = 13 + 3 * i
    ...
    # 37
    x = darknet_conv(
        x, filters=512, kernel_size=(3, 3), strides=(2, 2), padding="valid", idx=37
    )
    ...
    # 62
    x = darknet_conv(
        x, filters=1024, kernel_size=(3, 3), strides=(2, 2), padding="valid", idx=62
    )
    ...
    x = darknet_conv(
        x, filters=256, kernel_size=(1, 1), strides=(1, 1), padding="same", idx=84
    )
    x = UpSampling2D(2)(x)
    ...
    x = darknet_conv(
        x, filters=128, kernel_size=(1, 1), strides=(1, 1), padding="same", idx=96
    )
    x = UpSampling2D(2)(x)
